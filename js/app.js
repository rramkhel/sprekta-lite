        ...
        async function loadProfile() {
            try {
                ...
                if (result && result.value) {
                    userProfile = JSON.parse(result.value);
                }
            } catch (error) {
...
Reference the profile naturally in responses when relevant. Respond with ONLY the JSON object.`,
        ...
        async function loadAllNotes() {
            try {
                const result = await window.storage.get('all-notes');
                if (result && result.value) {
        ...
        async function saveAllNotes() {
            try {
                await window.storage.set('all-notes', JSON.stringify(notes));
            } catch (error) {
        ...
        async function submitQuickCapture() {
            ...
            try {
                let captures = [];
                try {
                    const result = await window.storage.get('quick-captures');
                    if (result && result.value) {
                        captures = JSON.parse(result.value);
                    }
                } catch (e) {}
                ...
                await window.storage.set('quick-captures', JSON.stringify(captures));
            } catch (error) {
